...
// initial values
setTimeout(function () {
  inputBus.push([{ text: "おはようございます！", time: Date.now() - 60 * 60 * 6 * 1000 }, { text: "こんにちは！", time: Date.now() - 60 * 60 * 3 * 1000 }, { text: "こんばんは！", time: Date.now() }].reverse());
}, 500);
...
module.exports = {
  relativeTime: function (time, now) {
    ...
    return day_diff === 0 && (diff < 60 && Math.floor(diff) + "秒前" || diff < 120 && "1分前" || diff < 3600 && Math.floor(diff / 60) + "分前" || diff < 7200 && "1時間前" || diff < 86400 && Math.floor(diff / 3600) + "時間前") || day_diff === 1 && "昨日" || day_diff < 7 && day_diff + "日前" || day_diff < 31 && Math.floor(day_diff / 7) + "週間前";
  }
};
